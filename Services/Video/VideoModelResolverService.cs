...
{
    public class VideoModelResolverService : IVideoModelResolverService
    {
        ...
        public VideoModelResolverService(
        ...
        public async Task<ResolvedVideoPreset> ResolvePresetAsync(string presetId, JsonDocument? requestParams)
        {
            var preset = await _presetRepository.GetByIdAsync(presetId)
                         ?? throw new ValidationAppException($"Preset '{presetId}' not found");

            if (preset.Status != VideoModelPresetStatus.Active)
            {
            ...
            if (model.Status == VideoModelStatus.Archived)
            {
            ...
            if (version.Status == VideoModelVersionStatus.Archived)
            {
            ...
            return new ResolvedVideoPreset(model, version, preset, resolvedParams);
        }

        public async Task<ResolvedVideoModelVersion> ResolveModelAndVersionAsync(string modelSlug, string? versionTag, JsonDocument? requestParams)
        {
            ...
            return new ResolvedVideoModelVersion(model, version, resolvedParams);
        }

        private BsonDocument MergeAndValidate(VideoModelVersion version, VideoModelPreset? preset, JsonDocument? requestParams)
        {
